...
\addplot[color=colRed, mark=*]
  coordinates {(0.721,328003)(0.873,218085)(0.969,130347)(0.994,91457)(0.998,58703)(0.998,41107)(0.998,31076)};
\addlegendentry{PyNear \texttt{HNSWL2Index}}
\addplot[color=colRed, dashed, mark=o]
  coordinates {(0.688,722626)(0.811,483515)(0.871,303362)(0.885,180001)(0.889,103056)(0.889,72326)(0.889,62117)};
...
\caption{HNSW recall--throughput frontiers ($N = 10^5$, $d = 128$,
  ...
  left to right on each curve).  Faiss leads both like-for-like pairs by
  $1.4$--$1.8\times$ at matched recall.  PyNear's
  \texttt{HNSWL2IndexSQ8} nevertheless outruns even Faiss's \emph{float}
  index below ${\sim}0.87$ recall at a quarter of the memory; its recall
  ceiling (0.889 vs.\ 0.944) reflects the global symmetric int8 scale
  versus Faiss's per-dimension trained quantiser.}
\label{fig:hnsw-pareto}
\end{figure}

Faiss wins both like-for-like comparisons: \texttt{IndexHNSWFlat}
delivers $1.4$--$1.7\times$ the throughput of \texttt{HNSWL2Index} at
matched recall (203\,k vs.\ 130\,k QPS at $\mathrm{Recall}@10 \approx
0.97$; 130\,k vs.\ 91\,k at ${\approx}0.995$) with essentially identical
recall-per-$\mathrm{ef}$ (the two float curves overlap in recall at every
sweep point, confirming equivalent graph construction quality), and
\texttt{IndexHNSWSQ} leads \texttt{HNSWL2IndexSQ8} by a similar margin
while reaching a higher quantised recall ceiling (0.944 vs.\ 0.889).
Build times are comparable (float: 1.16\,s vs.\ 1.02\,s; quantised:
0.68\,s vs.\ 0.84\,s, PyNear faster).  Two honest observations follow.
First, PyNear's SQ8 index is the fastest option of the four below
${\sim}0.87$ recall — ahead of Faiss's float index at $4\times$ less
vector memory — making it a genuinely competitive operating point for
recall-tolerant, memory-tight workloads.  Second, the quantised recall
gap is a known, fixable design difference: PyNear quantises with a single
global symmetric scale, Faiss trains per-dimension bounds; adopting
per-dimension quantisation is on the roadmap.  Where PyNear's HNSW family
differentiates itself today is capability rather than raw float
