...
replacing the previous per-cluster VP-Tree traversal, and
...
$2.4\text{--}3.7\times$ build-time speedup.
...
\texttt{VPTreeL2Index} achieves query latencies $2\text{--}58\times$
lower than scikit-learn's kd-tree at low to moderate dimensionalities
($d \le 128$).  In the high-dimensionality approximate regime
...
\addplot[color=colOrange, mark=square*]
  coordinates {(2,0.9)(3,0.9)(4,0.9)(5,1.0)(6,1.1)(7,1.1)(8,1.2)(16,2.5)};
\addlegendentry{Faiss \texttt{IndexFlatL2}}

% scikit-learn
\addplot[color=colGreen, mark=triangle*]
  coordinates {(2,0.4)(3,0.6)(4,0.6)(5,0.5)(6,1.1)(7,1.2)(8,2.2)(16,5.2)};
\addlegendentry{scikit-learn}

% PyNear VPTreeL2Index
\addplot[color=colRed, mark=diamond*]
  coordinates {(2,0.4)(3,0.4)(4,0.4)(5,0.5)(6,0.6)(7,0.5)(8,0.7)(16,1.0)};
...
\caption{Mean query latency (ms) vs.\ dimensionality for exact $\ell_2$
  ...
  scikit-learn across the entire range, staying below 1.0 ms up to $d=16$.}
\label{fig:l2-low}
\end{figure}

\texttt{VPTreeL2Index} achieves the lowest latency among exact methods
throughout the low-dimensional range, with query times between 0.4 ms and
1.0 ms.  scikit-learn's kd-tree is competitive at $d = 2$ but grows
steadily with $d$, reaching 5.2 ms at $d = 16$.  Faiss's flat index incurs
a relatively constant overhead (reflecting its BLAS-driven batch computation)
that becomes expensive at $d = 16$ (2.5 ms).  Annoy's approximate index
...
% Faiss IndexFlatL2
\addplot[color=colOrange, mark=square*]
  coordinates {(32,4.3)(64,8.9)(128,22.0)(256,43.6)(512,86.8)};
\addlegendentry{Faiss \texttt{IndexFlatL2}}

% scikit-learn
\addplot[color=colGreen, mark=triangle*]
  coordinates {(32,15.6)(64,47.7)(128,65.6)(256,67.7)(512,582.3)};
\addlegendentry{scikit-learn}

% PyNear VPTreeL2Index
\addplot[color=colRed, mark=diamond*]
  coordinates {(32,1.0)(64,1.4)(128,1.6)(256,3.4)(512,7.3)};
\addlegendentry{PyNear \texttt{VPTreeL2Index}}
...
\caption{Mean query latency (ms) vs.\ dimensionality for exact $\ell_2$
  ...
  582 ms at $d=512$; Faiss scales to 87 ms.  \texttt{VPTreeL2Index} maintains
  near-linear growth from 1.0 ms at $d=32$ to 7.3 ms at $d=512$, an
  $80\times$ advantage over scikit-learn and a $12\times$ advantage over Faiss
  at the highest tested dimensionality.}
\label{fig:l2-high}
\end{figure}

The results reveal a stark divergence in scaling behaviour.  scikit-learn's
latency grows super-linearly with $d$, reaching 67.7 ms at $d=256$ and
582.3 ms at $d=512$.  Faiss's flat index grows more gracefully (87 ms at
$d=512$) owing to its vectorised batched scan.  \texttt{VPTreeL2Index} is the
most efficient exact method at all tested dimensionalities, growing from 1.0 ms
at $d=32$ to only 7.3 ms at $d=512$ — a $12\times$ advantage over Faiss
and an $80\times$ advantage over scikit-learn at the highest dimensionality.
This reflects the tree's ability to prune branches via the triangle inequality:
even as the intrinsic dimensionality of the data grows, the clustered structure
allows the VP-tree to eliminate a significant fraction of candidates without
explicitly computing their distances.
...
% Faiss IndexFlatL2
\addplot[color=colOrange, mark=square*]
  coordinates {(10000,0.1)(50000,0.4)(100000,1.2)(250000,2.2)(500000,24.2)(1000000,49.9)};
\addlegendentry{Faiss \texttt{IndexFlatL2}}

% PyNear VPTreeL2Index
\addplot[color=colRed, mark=diamond*]
  coordinates {(10000,1)(50000,8)(100000,17)(250000,51)(500000,146)(1000000,329)};
\addlegendentry{PyNear \texttt{VPTreeL2Index}}
...
\caption{Exact-index build time (ms, log--log) vs.\ dataset size $N$, $d=32$.
  ...
  curve, from 1 ms at $N=10\,000$ to 329 ms at $N=10^6$ — well within
...
$6\text{--}7\times$ overhead vs.\ Faiss's copy-only flat index at large $N$,
which is expected and acceptable: the VP-Tree build cost is paid once and
amortised over all subsequent queries, which benefit from $O(\log N)$
traversal rather than $O(N)$ linear scan.
...
% Faiss IndexIVFFlat
\addplot[color=colOrange, mark=square*]
  coordinates {(10000,137)(50000,658)(100000,1319)(250000,3116)(500000,6543)(1000000,14098)};
\addlegendentry{Faiss \texttt{IndexIVFFlat}}

% PyNear IVFFlatL2Index (C++ K-Means++)
\addplot[color=colRed, mark=diamond*]
  coordinates {(10000,29)(50000,530)(100000,1837)(250000,9464)(500000,26488)(1000000,69897)};
...
\midrule
10\,000  & \textbf{29}    & 137    \\
50\,000  & \textbf{530}   & 658    \\
100\,000 & 1\,837  & \textbf{1\,319}  \\
250\,000 & 9\,464  & \textbf{3\,116}  \\
500\,000 & 26\,488 & \textbf{6\,543}  \\
1\,000\,000 & 69\,897 & \textbf{14\,098} \\
\bottomrule
\end{tabular}
\end{table}
...
(7.3 ms vs.\ 86.8 ms for Faiss flat, 582.3 ms for scikit-learn).  The tree's
...
low-to-moderate dimensionality regime ($d \leq 512$) without requiring BLAS,
