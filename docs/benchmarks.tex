...
approximate index with SIMD-accelerated distance kernels.  Version 2.1 introduces a
...
Build time is $3\text{--}4\times$ higher than Faiss IVF owing to
NumPy K-Means vs.\ Faiss's compiled K-Means.  All experiments
...
\texttt{IVFFlatL2Index} uses scikit-learn's \texttt{MiniBatchKMeans} for
clustering (falling back to a pure-NumPy Lloyd's K-Means when scikit-learn is
unavailable); Faiss \texttt{IndexIVFFlat} uses its own highly optimised
compiled K-Means.

\begin{table}[h]
\centering
\caption{Index build time (ms), $N = 50\,000$, averaged over 3 runs.}
\label{tab:build}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{lcccc}
\toprule
...
& 128 & 256 & 512 & 1024 \\
\midrule
\textbf{IVFFlatL2Index} & 2670 & 3401 & 5552 & 9850 \\
\textbf{Faiss IndexIVFFlat} & 675 & 1121 & 2026 & 3805 \\
\bottomrule
\end{tabular}
\end{table}

Faiss IVFFlat builds $3\text{--}4\times$ faster than \texttt{IVFFlatL2Index}
across all dimensionalities, reflecting its compiled K-Means implementation.
Build time grows roughly linearly with $d$ for both methods.  In workloads
where the index is built once and queried many times (e.g.\ embedding
retrieval in production), this build-time difference is typically amortised
quickly.
...
lower query latency.  Build time is $3\text{--}4\times$ higher for
\texttt{IVFFlatL2Index} due to its NumPy-based K-Means vs.\ Faiss's
compiled implementation.  Practitioners requiring maximum query throughput
and willing to accept a compiled native dependency should prefer Faiss IVF;
\texttt{IVFFlatL2Index} is preferable when a pure-Python install
(NumPy only) or the exact-to-approximate fallback via a single parameter
change are operationally valuable.
...
and $d=512$ while maintaining perfect Recall@10 at $n_{\mathrm{probe}} = 20$.
Build time is $3\text{--}4\times$ higher than Faiss IVF owing to
NumPy K-Means vs.\ Faiss's compiled implementation.
