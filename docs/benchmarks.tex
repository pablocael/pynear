...
\author{PyNear Development Team \\[4pt]
        ...
        \small Third edition — PyNear version 2.5}
\date{July 2026}
...
PyNear version 2.5, whose exact indices gained \emph{leaf bucketing}:
...
For exact Hamming search, Faiss \texttt{IndexBinaryFlat} remains the
fastest method at every tested width when measured fairly in its own
process (0.15--0.29 ms vs.\ 7.6--15.9 ms for \texttt{VPTreeBinaryIndex});
the apparent 256-bit win for the VP-tree in the in-process suite is an
artefact of the libgomp/libomp contention discussed in
...
Version 2.5, evaluated in this edition, reworks the exact-index hot paths:
...
\caption{Mean query latency (ms) vs.\ dimensionality for exact $\ell_2$
  ...
  Despite the five-fold larger dataset, \texttt{VPTreeL2Index} (v2.5, leaf
...
v2.5 leaf-bucketed traversal.  scikit-learn's kd-tree is competitive at
...
data combined with the v2.5 leaf-bucketed SIMD sweeps.  Note that this
...
\caption{Mean query latency (ms) vs.\ descriptor length for exact
  ...
  Faiss is measured in-process here, which depresses its popcount scan
  severely (OpenMP caveat, Section~\ref{sec:setup}): measured in an
  isolated process on the same dataset it answers the batch in 0.15 ms
  (128-bit) and 0.29 ms (256-bit) and remains the fastest exact method
  at every width; the apparent VP-tree win at 256 bits is a same-process
  artefact.
...
The binary experiments have one dominant conclusion: exact Hamming search
is brute force's home turf.  Faiss's batched hardware-popcount scan,
measured fairly in its own process, answers the 16-query batch in
0.15--0.29 ms across all widths — well ahead of
\texttt{VPTreeBinaryIndex} (3.2--15.9 ms), whose metric-tree pruning
cannot cut deeply into uniformly random binary codes.  The in-process
Faiss curve in Figure~\ref{fig:binary} crosses above the VP-tree at 256
bits (36.3 ms), but that crossing is entirely explained by the OpenMP
runtime contention of Section~\ref{sec:setup} — it is reproduced here
because same-process deployment is common in practice, not because the
tree wins fairly.  PyNear's competitive binary offerings are therefore
the approximate/radius indices of Section~\ref{sec:approx-binary} (MIH
and binary IVF), not exact tree search.  A further note:
\texttt{BKTreeBinaryIndex} is
...
% =============================================================================
\section{Graph-Based Search: HNSW vs.\ Faiss HNSW}
\label{sec:hnsw}

PyNear's HNSW family (\texttt{HNSWL2Index} and the int8-quantised
\texttt{HNSWL2IndexSQ8}) is compared against its direct Faiss counterparts,
\texttt{IndexHNSWFlat} and \texttt{IndexHNSWSQ} (8-bit scalar quantiser), on
$N = 100\,000$ clustered 128-D float vectors with queries drawn from the
same cluster distribution: identical data, $M = 16$,
$\mathrm{ef}_{\mathrm{construction}} = 200$, batches of 1\,000 queries at
$k = 10$ on 24 threads for \emph{both} systems, sweeping
$\mathrm{ef}_{\mathrm{search}} \in \{16, \dots, 1024\}$ (best of 5 runs per
point).  Faiss is measured in a Faiss-only subprocess
(Section~\ref{sec:setup}); reproduce with
\texttt{python -m pynear.benchmark.hnsw\_faiss\_benchmark}.
...
\begin{axis}[
  ...
  ymode=log,
  xmin=0.66, xmax=1.02,
  ymin=20000, ymax=2000000,
  xlabel={Recall@10},
  ylabel={Queries per second (log scale)},
  legend style={at={(0.03,0.03)}, anchor=south west},
  legend cell align=left,
]
\addplot[color=colRed, mark=*]
  coordinates {(0.717,359084)(0.868,217249)(0.964,131038)(0.991,85711)(0.996,58235)(0.996,39090)(0.996,30712)};
\addlegendentry{PyNear \texttt{HNSWL2Index}}
\addplot[color=colRed, dashed, mark=o]
  coordinates {(0.693,568756)(0.838,401026)(0.911,291061)(0.936,174511)(0.940,100730)(0.940,75773)(0.940,60775)};
\addlegendentry{PyNear \texttt{HNSWL2IndexSQ8}}
\addplot[color=colOrange, mark=square*]
  coordinates {(0.726,553821)(0.875,335004)(0.967,203156)(0.995,129842)(1.000,90634)(1.000,61006)(1.000,28928)};
\addlegendentry{Faiss \texttt{IndexHNSWFlat}}
\addplot[color=colOrange, dashed, mark=square]
  coordinates {(0.715,1184604)(0.848,731426)(0.921,438913)(0.942,277383)(0.944,155581)(0.944,92057)(0.944,38463)};
\addlegendentry{Faiss \texttt{IndexHNSWSQ} (8-bit)}
\end{axis}
\end{tikzpicture}
\caption{HNSW recall--throughput frontiers ($N = 10^5$, $d = 128$,
  $k = 10$, $M = 16$, 24 threads for both systems, log-scale QPS; markers
  are $\mathrm{ef}_{\mathrm{search}} = 16, 32, 64, 128, 256, 512, 1024$
  left to right on each curve).  Faiss leads both like-for-like pairs at
  matched recall, but PyNear's \texttt{HNSWL2IndexSQ8} — per-dimension
  affine quantisation with asymmetric search as of this edition — now
  tracks or beats Faiss's \emph{float} index up to ${\sim}0.91$ recall
  at a quarter of the vector memory, with a quantised recall ceiling of
  0.940 vs.\ Faiss's 0.944.}
\label{fig:hnsw-pareto}
\end{figure}

Faiss wins both like-for-like comparisons, but the margins tightened in
this edition.  \texttt{IndexHNSWFlat} delivers $1.5$--$1.6\times$ the
throughput of \texttt{HNSWL2Index} at matched recall (203\,k vs.\
131\,k QPS at $\mathrm{Recall}@10 \approx 0.97$) with essentially
identical recall-per-$\mathrm{ef}$ — the two float curves overlap in
recall at every sweep point, confirming equivalent graph construction
quality.  In the quantised pair, PyNear's \texttt{HNSWL2IndexSQ8} was
reworked for this edition: per-dimension affine int8 quantisation with
\emph{asymmetric} search (float query against decoded codes, the Faiss
ADC approach), chosen by simulating four quantisation schemes against
exhaustive-search ground truth.  Its recall ceiling rose from 0.889 to
0.940 (Faiss: 0.944; the scheme's own exhaustive ceiling is 0.941, which
the graph attains), while a concurrently landed allocation-free search
path (pooled per-thread scratch, results written directly into the
output buffers) absorbed the decode-kernel cost — SQ8 throughput at
$\mathrm{ef} \ge 64$ matches the previous lower-recall kernel.  Faiss's
\texttt{IndexHNSWSQ} remains ${\sim}1.5$--$2\times$ faster at matched
recall, but PyNear's SQ8 now tracks or beats Faiss's \emph{float} index
up to ${\sim}0.91$ recall at $4\times$ less vector memory — a genuinely
competitive operating point for recall-tolerant, memory-tight
workloads.  Build times are comparable (float: 1.22\,s vs.\ 1.02\,s;
quantised: 0.67\,s vs.\ 0.84\,s, PyNear faster).  Where PyNear's HNSW
family differentiates itself is capability rather than raw float
throughput: a binary/Hamming HNSW, the MIH-seeded variant of
Section~\ref{sec:approx-binary}, in-place \texttt{add}/\texttt{remove}/%
\texttt{rebuild} with filtered search, and per-shard pickling via
\texttt{ShardedHNSWIndex}.
...
$34\times$ figure reported here.)  Figure~\ref{fig:binary-512-qps}
visualises the four methods on a logarithmic throughput axis.
...
\begin{axis}[
  ...
  ymode=log,
  log origin=infty,
  ymin=10, ymax=2000000,
  ylabel={Queries per second (log scale)},
  symbolic x coords={PyNear MIH, PyNear IVF, Faiss Flat, Faiss MIH},
  xtick={PyNear MIH, PyNear IVF, Faiss Flat, Faiss MIH},
...
\addplot[ybar, bar shift=0pt, fill=colRed!85,    draw=colRed!50!black]
  coordinates {(PyNear MIH, 114039) [114\,039]};
\addplot[ybar, bar shift=0pt, fill=colBrown!75,  draw=colBrown!50!black]
  coordinates {(PyNear IVF, 47993) [47\,993]};
\addplot[ybar, bar shift=0pt, fill=colOrange!85, draw=colOrange!50!black]
  coordinates {(Faiss Flat, 3341) [3\,341]};
\addplot[ybar, bar shift=0pt, fill=colPurple!75, draw=colPurple!50!black]
  coordinates {(Faiss MIH, 46) [46]};
\end{axis}
\end{tikzpicture}
\caption{Throughput at 100\% Recall@10 for 512-bit near-duplicate
  retrieval ($N = 10^6$, $k=10$, 24 threads; data of
  Table~\ref{tab:binary-512}, log scale).  PyNear
  \texttt{MIHBinaryIndex} sustains $34\times$ the throughput of Faiss's
  subprocess-isolated exact scan; Faiss's own MIH implementation is
  ${\sim}2{,}500\times$ slower at this descriptor width.}
\label{fig:binary-512-qps}
\end{figure}
...
(Table~\ref{tab:mih-vs-faiss-mih}); Figure~\ref{fig:sift-pareto} shows the
full recall--throughput landscape.
...
\addplot[color=colRed, mark=*]
  coordinates {(0.466,38554)(0.652,18158)(0.799,7326)(0.832,4254)(0.841,1541)};
\addlegendentry{PyNear \texttt{MIHBinaryIndex} ($m=8$, $r=4..24$)}
\addplot[color=colBrown, mark=square*]
  coordinates {(0.825,125776)(0.842,87783)(0.845,56859)};
\addlegendentry{PyNear \texttt{IVFFlatBinaryIndex} ($n_{\mathrm{probe}}=31..125$)}
\addplot[color=colPurple, mark=triangle*]
  coordinates {(0.28,52346)(0.53,15881)(0.73,3456)(0.82,689)(0.84,166)};
\addlegendentry{Faiss \texttt{IndexBinaryMultiHash} ($n_{\mathrm{flip}}=1..5$)}
\addplot[color=colOrange, dashed, no marks, domain=0.24:0.88] {23572};
\addlegendentry{Faiss \texttt{IndexBinaryFlat} (exact, 23\,572 QPS)}
\end{axis}
\end{tikzpicture}
\caption{SIFT1M 128-bit: throughput vs.\ Recall@10 ($N = 10^6$, $k = 10$,
  500 queries, 24 threads; log-scale QPS).  Up and to the right is better.
  Each marker is one measured operating point from
  Tables~\ref{tab:binary-sift} and~\ref{tab:mih-vs-faiss-mih}.  PyNear's
  MIH sweep dominates Faiss's \texttt{IndexBinaryMultiHash} across the
  curve, and \texttt{IVFFlatBinaryIndex} dominates everything at the
  ${\sim}0.845$ tie-limited recall ceiling; the dashed line is Faiss's
  subprocess-isolated exact scan, which beats both MIH implementations
  above the ${\sim}0.5$ recall mark — the caveat stated in the text.}
\label{fig:sift-pareto}
\end{figure}
...
evaluations, and with v2.5's 32-point leaf buckets the evaluations that do
...
every tested width when measured in its own process (0.15--0.29 ms for
the 200k-code suite, vs.\ 3.2--15.9 ms for \texttt{VPTreeBinaryIndex}).
The in-process suite shows the VP-tree ahead at 256 bits, but that
ordering is an artefact of the OpenMP runtime clash
(Section~\ref{sec:setup}).  For exact binary search, prefer a flat
popcount scan; PyNear's binary strengths are MIH and binary IVF
(Section~\ref{sec:approx-binary}).
